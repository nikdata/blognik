...
The {reticulate} package is a clever package that enables you to 'store' Python libraries and 'call' Python functions using familiar R syntax. My daily workflow consists of analyzing data through R and with the power of the {tidyverse}, data wrangling and analysis is superior to that of Pandas and Matplotlib on Python. However, I have found that Scikit-Learn on Python is fairly robust machine learning package as compared to the various packages available through {tidymodels}.

Perhaps the most important benefit (to me at least) about Scikit-Learn is its innate ability to use parallelization (i.e., multi-core processing) to build a variety of models without the need to [load additional libraries](https://tune.tidymodels.org/articles/extras/optimizations.html). In R, I find that many algorithms aren't as intuitive in leveraging all available cores. And this is key when I'm building a model and I want to throw as much computing power as my machine can muster.

## TL;DR

This is a LONG post. You can find all the code used on my [GitHub as a gist](https://gist.github.com/nikdata/ce4c72550f184e6bc20d6c98475e5a5d).

## My tutorial on {reticulate}

There is a tremendous amount of information on the web about {reticulate}. While that information is great, I found that there are not many tutorials out there that really show off the power of leveraging the best of R and Python in one R script. My goal is to walk through a simple dataset leveraging the following:

- {ggplot2} for graphics
- {recipes} for preprocessing data
- {reticulate} for using various Python libraries
  - imblearn: resampling to balance data
...
Luckily, the information we want is encoded correctly. For instance, the categorical columns are already encoded as factors. However, we need to remember that Scikit-Learn does not accept character values. Rather, these must be encoded as numerical values (either ordinally, arbitrarily, dummy-encoded, or one-hot encoding). And we'll do that when we get to our recipes.
